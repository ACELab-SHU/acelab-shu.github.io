...
window.U = (() => {
  ...
  function bindCanvas(canvas) {
    ...
    return { canvas, fit, ctx: canvas.getContext("2d") };
  }
  ...
  function showDrill({ title, value, delta, sub, source, x, y }) {
    if (!drillOpen) drillTrigger = document.activeElement;
    const closeLabel = document.documentElement.lang.startsWith("en") ? "Close details" : "关闭详情";
    drill.innerHTML = `<button class="d-close" aria-label="${closeLabel}">✕</button>
      <div class="d-title" id="drill-title">${title}</div>
    ...
    drill.querySelector(".d-close").focus({ preventScroll: true });
  }
  function hideDrill() {
    drill.hidden = true; drillOpen = false;
    if (drillTrigger && drillTrigger.isConnected) drillTrigger.focus({ preventScroll: true });
  }
  document.addEventListener("keydown", e => {
    if (drillOpen && e.key === "Escape") { e.preventDefault(); hideDrill(); }
  });
